...
# M scaling configuration: ~0.966B total parameters and ~0.255B active
# Transformer-body parameters per token. Keep this experiment isolated from
# the existing L runs even if OUTPUT_ROOT is exported in the shell.
export OUTPUT_ROOT="${M_OUTPUT_ROOT:-/mnt/workspace/fmoe_cuda_m_8e_outputs}"
export MAX_STEPS="${M_MAX_STEPS:-152000}"
...
latest_common_checkpoint_step() {
  ...
  comm -12 "${tmp_dir}/baseline.steps" "${tmp_dir}/proposed.steps" \
    > "${tmp_dir}/common.steps"
