...
TRANSFER_CSV = os.path.join(
    os.path.dirname(__file__),
    "results_band_attribution_seq1024",
    "a_to_h_transfer_summary_layers1_27.csv",
)
...
with open(TRANSFER_CSV, newline="", encoding="utf-8") as handle:
    a_top1 = next(
        row for row in csv.DictReader(handle)
        if row["a_band"] == "0_1" and row["h_band"] == "0_1"
    )
...
continuity = [
    ...
    float(a_top1["a_band_adjacent_cosine"]),
]
source_fraction = [
...
a_color = "#D65A3A"
a_top_color = "#8F2D1F"
colors = [x_color, a_color, a_top_color]
...
for index, (axis, (values, title, ylabel, ylim, formatter)) in enumerate(zip(axes, panels)):
    labels = ["Residual X", "Attention A", "A top 1%"] if index == 0 else ["Residual X", "Attention A"]
...
axes[0].annotate(
    "Continuity concentrates\nin A's top subspace",
    xy=(2, continuity[2]),
    xytext=(0.55, 0.675),
    arrowprops={"arrowstyle": "->", "color": a_top_color, "lw": 1.5},
    color=a_top_color,
    fontsize=10,
    fontweight="bold",
)
