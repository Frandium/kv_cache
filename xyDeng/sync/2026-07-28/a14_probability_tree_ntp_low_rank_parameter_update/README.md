...
三份源理论文件的组合 SHA-256：
`cd21ffbdff76f7970ec409643cc9e71a35f8b5323a2a1809b5ba35bc12e0fcbf`
...
记 $q_0:=kr$ 为注册秩预算，$q_\star:=\dim U_\star\le q_0$ 为实际任务维数。则

$$
\operatorname{rank}(\Delta W_T)\le q_\star\le q_0=kr,
\qquad
...
- 受控固定 mapping injections 给出 $q_\star:=\dim U_\star\le q_0:=kr$；
- NTP exposure 的相干增长下界；
- 循环 pure SGD 使 exposure 与低维 task spike 发散；
- 固定维数精确模型满足 $\tau_{q_0}(W_T)=\tau_{kr}(W_T)\to0$。

### 条件证明

- 小 conditional bias / martingale leakage 下的 $\|R_T\|_F/\|S_T\|_F\xrightarrow{\mathbb P}0$；
- multiclass softmax；
