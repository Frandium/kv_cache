...
**文档状态：**自包含理论草案 v0.3；实验 Protocol 保持待审批
...
为说明 NTP 不只是外积梯度的背景，我们进一步定义 mapping exposure。对有限的 contrast-NTP 事件集，若每个事件对同一 separator 具有正 margin，并在 pure SGD 中获得无限累计步长质量，则仅由参数路径长度和 logistic pressure 的正性即可推出 pressure-weighted exposure 与结构化 task spike 发散。精确受控情形中 \(R_T=0\)；近似情形若空间外偏差和 martingale 波动满足 \(R_T=o_{\mathbb P}(\|S_T\|_F)\)，则

$$
\Delta W_T=S_T+R_T,
\qquad
\operatorname{rank}(S_T)\le kr,
\qquad
\|S_T\|_F\to\infty.
$$
...
对任意非零矩阵 \(A\)，定义 rank-\(q\) 外相对谱能量

$$
\tau_q(A)
:=
\frac{\sum_{j>q}\sigma_j(A)^2}{\|A\|_F^2}.
$$
...
r_{1-\varepsilon}(A)
:=
\min\{q:\tau_q(A)\le\varepsilon\}.
$$

令 \(n=\min(d,m)\)，并假设注册预算满足

$$
q=kr\le n.
$$

\(\tau_{kr}\) 是主指标；能量秩是等价表述。entropy effective rank 只作固定维数推论。

## 1.3 必须区分的五个对象

| 对象 | 本文结论 |
|---|---|
| \(\operatorname{rank}(\Delta W_T)\) | 精确共享包络下至多为 \(kr\) |
| \(\tau_{kr}(\Delta W_T)\) | 近似共享下由 \(R_T/S_T\) 控制 |
| \(\operatorname{rank}(W_T)\) | 不声称低；Gaussian 基座几乎必然满秩 |
| \(W_T\) 的 task outliers | task spike 超过算子范数 bulk 门时出现 |
| \(\tau_{kr}(W_T)\) | task spike 在 Frobenius 能量上主导时趋零 |
...
$$
\operatorname{rank}(S_T)\le\dim U_\star\le q=kr
$$
...
\sum_{t<T}\eta_t
\sum_{e\in\mathcal B_t}
\omega_{e,t}r_e(W_t)X_e.
$$

每个 \(X_e\) 的行空间位于 \(U_\star\)，所以 \(S_T\) 的秩不超过 \(q\)。再与 \(Q_\star\) 取 Frobenius 内积：

$$
\langle S_T,Q_\star\rangle_F
=
\sum_{t,e}
\eta_t\omega_{e,t}r_e(W_t)
...
$$
s_T:=\langle S_T,Q_\star\rangle_F.
$$
...
$$
r_e(W)=\frac{1}{1+\exp\!\bigl(\langle W,X_e\rangle_F\bigr)}
$$

在每个有限 \(W\) 上连续且严格为正，因此存在 \(c>0\)，使充分大 \(t\) 上所有事件均满足 \(r_e(W_t)\ge c\)。于是任取事件 \(e\)，

$$
\mathsf A_\infty
\ge
\sum_{t\ge t_0:e_t=e}\eta_t r_e(W_t)
\ge
c\sum_{t\ge t_0:e_t=e}\eta_t
=\infty,
$$

矛盾。故 \(\mathsf A_T\to\infty\)。T2 随即给出 \(s_T\to\infty\) 与 \(\|S_T\|_F\to\infty\)，而 \(\mathsf E_T^{\mathrm{map}}\ge\mathsf A_T\)；秩上界仍由 T1 与 pure-SGD 行空间保持得到。证毕。
...
令 \(\Gamma_T:=\gamma\mathsf A_T\)。若存在确定性尺度 \(g_T\uparrow\infty\)，使
...
由 Markov 不等式，该随机项为 \(o_{\mathbb P}(g_T)\)；bias 项至多为 \(B_T=o_{\mathbb P}(g_T)\)。T2 又给出 \(\|S_T\|_F\ge\Gamma_T\ge g_T\) 以趋一概率成立。证毕。
...
\sigma_q(W_T)
\ge
\sigma_q(S_T)-\|N_T\|_{\mathrm{op}}.
$$

因此若

$$
\sigma_q(S_T)>2\|N_T\|_{\mathrm{op}},
$$

则

$$
\sigma_q(W_T)>\sigma_{q+1}(W_T),
$$
...
奇异值扰动不等式给出前两式。对谱尾，\(S_T\) 是一个 rank-\(q\) 候选近似；最佳 rank-\(q\) 逼近恒等式给出

$$
\sum_{j>q}\sigma_j(W_T)^2
\le
\|W_T-S_T\|_F^2
=
\|N_T\|_F^2.
...
当 \(n=\min(d,m)\) 固定时，若 \(q<n\)、\(\tau_q(W_T)\to0\)，则

$$
\limsup_{T\to\infty}r_{\mathrm{eff}}(W_T)\le q.
$$

若宽度 \(n\) 与 \(T\) 同时增长，还需 \(\tau_q(W_T)\log n\to0\) 等联合控制，不能直接使用固定维数推论。
...
令注册预算 \(q=kr\)，且由 T1 有 \(\dim U_\star\le q\)。则：

$$
\operatorname{rank}(\Delta W_T)\le q
$$
...
T1 给出 \(\dim U_\star\le q=kr\)。NTP 外积梯度和 pure SGD 给出 \(\operatorname{Row}(\Delta W_T)\subseteq U_\star\)。T2--T3 给出 mapping exposure 的相干累积与 \(\|\Delta W_T\|_F\to\infty\)。精确受控情形 \(S_T=\Delta W_T\)、\(R_T=0\)，故
...
- 固定维数的精确模型中，发散 task spike 自动压过固定 Gaussian \(W_0\)，故完整 \(W_T\) 满足 \(\tau_{kr}(W_T)\to0\)；近似模型还需 \(R_T=o_{\mathbb P}(S_T)\)。
...
全文使用的推导均在本文或 [02_证明附录.md](02_证明附录.md) 内给出；假设与反例见 [03_物理先验与假设审计.md](03_物理先验与假设审计.md)。实验合同不属于本次 theory-only 同步。
