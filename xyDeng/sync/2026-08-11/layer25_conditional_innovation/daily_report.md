...
> 可访问性。其中，求解广义特征方程并据此构造目标条件方向的那部分实验，
> 本轮明确不进行人工审计，因为它并不特别直接地回答当前“怎样度量层写入
> 新增信息”的问题。二维充分性和后续秩阶梯同样不进入本报告的认识更新或
> 下一决策。
...
> **未纳入本轮审计的结果：**canonical 记录还包含求解广义特征方程所得的
> 目标条件方向、二维压缩和秩充分性指标。它们不直接裁定局部新增可访问性，
> 因而不进入上表、不改变局部 H2 结论，也不作为本报告的下一步。
