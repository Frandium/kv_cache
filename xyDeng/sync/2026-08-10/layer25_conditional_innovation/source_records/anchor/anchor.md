...
Thinking Card source: [researcher-owned Chinese card](thinking_card_cn.md). Chinese companion: [anchor](anchor_cn.md).
...
`A15_08_E02` (source-repository diagnostic record; not included in this minimal package)
...
`A15_08_E03` (source-repository diagnostic record; not included in this minimal package)
...
Unresolved: the smallest rank sufficient for this same $R_U$ object. Transfer,
native use, and routing function remain downstream.
...
**Exactly one next decision:** freeze one fresh-data rank-ladder design over
$r\in\{3,4,5,6,7\}$ on the same $R_U$ object, with equal-rank random and
TRAIN-label-permutation budgets and the unchanged 80% held-out-retention gate.
Its answer is the smallest passing rank, or “no sufficient compression through
rank seven” if none passes. This decision does not authorize Router training.
