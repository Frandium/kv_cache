...
Thinking Card：[研究者中文原始判断](thinking_card_cn.md)。英文正式版：[Anchor](anchor.md)。
...
`A15_08_E02`（源研究仓库中的诊断记录；本次最小同步包未纳入）
...
`A15_08_E03`（源研究仓库中的诊断记录；本次最小同步包未纳入）
...
尚未解决：同一个 $R_U$ 对象的最小充分秩。跨层/自然数据迁移、模型原生
使用和路由功能仍在后续。

不能声称：信息论创造、非线性新增、事实知识存储、MLP 原生使用、专家效用、Router 收益、自然语言普适性或逐层普遍规律。

**唯一下一决策：**在同一个 $R_U$ 对象上冻结一份全新数据的秩阶梯设计，
预注册 $r\in\{3,4,5,6,7\}$、同秩随机与 TRAIN 标签置乱预算，并保持 80%
留出增益保留门槛不变。输出是最小通过秩；若均不通过，则记录“七维以内没有
充分压缩”。这不授权 Router 训练。
