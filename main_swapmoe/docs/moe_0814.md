...
能力验证覆盖 M、L 两种模型尺度。各训练至约 $80$B tokens 后，M 模型八项下游任务平均为 $44.95\%$，baseline 为 $44.93\%$；L 模型平均为 $48.41\%$，baseline 为 $48.42\%$。两个尺度的差距分别为 $0.02$ 和 $0.01$ 个百分点，下游平均能力基本保持。
...
![Attention-Mean Routing 模型结构图](../assets/0814_attention_mean_routing_architecture.png)
...
在 ARC-Challenge、ARC-Easy、HellaSwag、LAMBADA OpenAI、PIQA、SIQA、RACE 和 WinoGrande 八项任务上进行统一 0-shot evaluation。
...
| HellaSwag     | 38.77%  | 37.57%    | -1.19 pp    | 45.88%    | 44.42%    | -1.45 pp |
| LAMBADA OpenAI| 42.32%  | 40.67%    | -1.66 pp    | 48.96%    | 46.92%    | -2.05 pp |
| PIQA          | 67.03%  | **67.52%**| +0.49 pp    | 69.53%    | **69.64%**| +0.11 pp |
| SIQA          | 39.41%  | 39.43%    | +0.03 pp    | 40.53%    | 40.97%    | +0.44 pp |
| RACE          | 50.06%  | 50.06%    | +0.01 pp    | 50.57%    | 51.12%    | +0.55 pp |
| WinoGrande    | 49.88%  | **51.22%**| +1.34 pp    | **51.70%**| 50.04%    | -1.66 pp |
| **Average**   | 44.93%  | **44.95%**| **+0.02 pp**| **48.42%**| 48.41%    | **-0.01 pp** |

M 与 L 模型的平均差分别为 $0.02$ 和 $0.01$ 个百分点。不同任务上存在正负波动，但总体差距稳定处于亚百分点范围，下游平均能力基本保持。

### 4.3 Expert 负载健康：连续性并非 Routing Collapse
...
### 4.4 长序列连续性：不同 Memory Budget 下均显著减少加载
...
#### 4.6.2 2048-token Prefill 与首字时延
...
1. **下游任务能力基本保持。** M、L 规模模型的八项任务宏平均分别仅相差 $0.02$ 和 $0.01$ 个百分点。
...
这些结果表明，Attention-Mean Routing 在两个模型尺度、约 $80$B 训练 tokens、多个 memory budgets 和真实按需加载系统上均保持一致效果，已经具备作为端侧 MoE 动态参数管理基础机制的实验依据。
