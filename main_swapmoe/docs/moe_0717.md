...
本阶段建立了 NTP 模型表征与参数空间的统一谱解释：residual 头部方向主要承载高频 token 与 common pattern，跨层稳定但跨 token 变化快；attention output 头部方向承载当前上下文的共享语义，具有更强的句内连续性。基于这一认识，我们使用 attention 头部连续成分进行 routing，在保持模型训练与下游能力基本稳定的同时，将专家切换次数降低了 $92.9\%$。
...
|---|---|---|---|---|---|
| Residual $X$ 的头部方向 | 高频 token 与 common pattern，例如冠词、助词、标点及高频搭配 | 对高频 token / pattern 区分性强；对 long-tail token 区分性弱 | 较弱，随高频 token identity 快速变化 | 强，dominant residual directions 沿深度持续继承 | 高频数据反复贡献同向梯度；语言嵌套结构使后续 token 持续复用这些方向 |
| Residual $X$ 的长尾方向 | 具体、低频的 token identity 与 long-tail semantic difference | 对高频 token 贡献较小；对 long-tail token 区分性强 | 整体较弱且稀疏 | 层间不同 | 低频数据的剩余误差信号写入尚未被头部方向占据的参数与表征方向 |
...
| Attention $A$ 的长尾方向 | 当前 token 经 QK 选择后获得的细粒度、低频上下文信息 | 对局部细节更敏感 | 随谱位置后移而明显降低 | 层间不同 | QK 权重差异选择不同 value，并保留 token-specific context residual |
...
### 3.1 方法：用局部均值在线近似 Attention 主方向
...
| 共同训练数据量 | 15.44B tokens | 15.44B tokens |
...
下图由两份训练日志重建，统一截取到共同的 step 58890，即 $15.44$B tokens；粗线为 100 个日志记录点的滑动平均。

![Residual routing 与 Attention-Top routing 的训练 Loss](../assets/0717_attention_top_routing_loss.png)

在共同区间末端，最近 500 个日志记录点的平均训练 loss 分别为 $3.1525$ 和 $3.1900$，两种 routing 保持在相同收敛区间。

### 3.3 Expert Activation 连续性：切换次数降低 92.9%

在连续 token 上统计 28 层 routed expert 的变化，Attention-Top routing 显著提高了 expert 驻留稳定性：

| Routing | 总 Expert 切换次数 | 相邻 Token 保持同一 Expert 的概率 |
|---|---:|---:|
| Residual routing | 1668.4 | 39.8% |
| Attention-Top routing | **118.8** | **95.7%** |

Expert 切换次数降低 $92.9\%$，stay probability 从 $39.8\%$ 提升至 $95.7\%$。这说明 attention 局部主方向不仅在表征空间中连续，而且能够被 router 转化为稳定的 expert activation；一次加载的 expert 可以跨多个相邻 token 复用，为 cache residency、预测预取和传输摊销提供了直接条件。

### 3.4 下游任务：总体能力基本保持

在共同 step 56000 的 checkpoint 上进行 0-shot 评估；此时每个模型累计训练约 $14.68$B tokens。结果如下：

| Task | Metric | Residual routing | Attention-Top routing | 差值 |
|---|---|---:|---:|---:|
| ARC-Challenge | acc_norm | 0.2679 | **0.2782** | +0.0102 |
| ARC-Easy | acc | 0.5467 | **0.5501** | +0.0034 |
| HellaSwag | acc_norm | **0.4134** | 0.3972 | -0.0161 |
| LAMBADA OpenAI | acc | **0.4328** | 0.4209 | -0.0118 |
| PIQA | acc | **0.6980** | 0.6882 | -0.0098 |
| RACE | acc | 0.5008 | **0.5012** | +0.0004 |
| SIQA | acc | 0.3920 | **0.3925** | +0.0005 |
| WinoGrande | acc | **0.5225** | 0.5201 | -0.0024 |
| **Average** | mean of listed metrics | **0.4718** | 0.4686 | **-0.0032** |

八项任务平均分仅相差 $0.0032$，即 $0.32$ 个百分点。结合 expert 切换次数降低 $92.9\%$ 的结果，本次实验验证了一个清晰结论：**以 attention 头部连续成分替代完整 residual 进行 routing，可以在基本保持语言模型能力的同时，将 token-level expert activation 转化为 context-level 稳定激活。**
