...
### 1.4 从 Frequency-Based MoE 到本轮结构

上次会议提出的 Frequency-Based MoE，核心是按照 attention output 的 feature 频率同时设计 routing 粒度与 expert 容量：

1. 将 attention output 的谱空间划分为高频、中频和低频 feature，并分别进行 routing；
2. 高频路径使用一个大 common expert，中频路径使用少量中等大小 experts，低频路径使用大量小 experts。

![Frequency-Based MoE](../assets/frequency_based_moe.svg)

该方案验证了按 feature 频率组织 routing 可以改善跨-token可预测性，但仍有约 $1\%$ 的任务性能差距。本轮研究进一步识别了其中的关键机制，并逐项修正模型结构：

| 设计问题 | 上次方案 | 本次修正 | 原因 |
|---|---|---|---|
| Routing 激活 | 高频、中频、低频 feature 分别独立 routing | 使用 Attention output 的谱空间主成份激活 long-tail expert |跨-token 保持强连续性的 routing 的信息主要来自 $A$ 的谱空间主成份，而非分散于三个频段中  |
| Expert 容量 | 高频使用大 common expert；频率越低，expert 越小、数量越多 | 使用小而常驻的 common expert，以及能够覆盖高 rank 非 common 空间的较大 long-tail experts | Common 写入的有效维度较低；单个 long-tail pattern 虽然局部低 rank，但全部 long-tail directions 的并集可以达到高 rank |
| 学习效果 | 未显式区分不同 experts 的输出职责 | 限制 long-tail experts 写入 common 正交补空间，使 common 与 long-tail 参数承担互补职责 | longtail token/pattern 须写入 longtail 空间以提升可分性，且其学习效率低下来自频率低。切分参数有助于降低干扰，提升学习效率。 |

因此，本轮结构不再依据 feature 频率机械缩放 expert，而是将两个机制分别用于正确的位置：**频率与 rank 结构决定参数如何组织，attention output 的连续性决定动态 expert 如何分发。**

## 2. 谱空间解释：为什么现有 MoE 呈现这种预测性与参数结构

本章根据自然语言的数据分布特点，和大语言模型的训练机制，解释现有 MoE 的两个直接现象：
...
> 1. 自然语言的 Zip'f 分布和嵌套结构导致高频 token 与 pattern 在表征和参数空间中形成少数高增益方向；
> 2. 现有 MoE gate 直接读取完整 token embedding，专家激活被这些强方向主导；
> 3. Expert 没有把可被复用的 commonn 计算与条件化增量拆开，Common 计算与 Longtail 计算占据同等数量参数，且重复学习。
...
二维 packed-Zipf 受控实验进一步展示了这一方向竞争过程：不同 groups 从同一局部区域初始化后，高频 common group 优先沿初始主方向形成稳定表示；其余低频 groups 则转向剩余方向，以获得彼此可区分的位置。频率决定了谁优先占据主方向，初始化几何则决定该主方向的具体朝向。
...
`the` 作为高频 token 首先占据了表征空间的强奇异方向，其后 `moon` 与 `the` 形成高频固定搭配的词组 `the moon`，导致 `moon` 的表征在 `the` 所在的强奇异方向上具有大投影。其后，模型再学习 `a moon cake` 等冷门搭配时，该低频词组中的低频 token `cake` 由于要利用前序所有 token 预测，不得不基于一个强 common 方向的分量调整 `cake` 的表征在空间中的位置。
...
跨层平均会平滑各层中更尖锐的 head/tail 分布。逐层统计中，`q_proj:L11`、`k_proj:L0`、`v_proj:L12` 和 `o_proj:L25` 的分布最为集中：少量 feature 保持高激活率，其余 feature 的激活率快速下降。下图以蓝色实线展示这些代表层，并以灰色虚线保留 28 层均值作为参照。
...
现有 MoE 的两类预测性来自同一个 gate 输入，却对应两种不同机制：跨-token预测性取决于相邻 token 在头部子空间中的可分性；同-token跨层预测性则来自 dominant residual directions 的持续继承。Attention output 提供了另一种更连续的 routing 信号，但并未主导当前 gate。

#### 2.3.1 跨 Token 可预测性差的原因

主导标准 gate 的 residual 强方向，由高频 token 和 common pattern 在训练早期率先形成。该头部子空间具有两种不同的区分性质：

1. **对高频 token 与 common pattern 区分性强。** Gate 能够从头部坐标中稳定读取 token identity，因此同一高频 token 倾向于进入同一个或同一组 expert。
2. **对 long-tail token 与 pattern 区分性弱。** Long-tail token 受 nested structure 影响，同样在 common 方向上具有较大投影；但真正区分不同 long-tail patterns 的信息主要位于后续谱方向，未能主导 gate。

因此，相邻 token 的 routing 难以稳定预测：高频 token 之间的头部坐标随 identity 显著变化，要准确预测其 expert activation，接近于直接预测下一 token；long-tail token 在头部子空间中缺乏稳定区分依据，因而会近似无固定语义对应地分散到其余 experts。高频与低频 token 在真实序列中交替出现，最终表现为较低的跨-token expert activation 可预测性。

#### 2.3.2 同 Token 跨层预测性好的原因

同一个 token 的表征会沿 residual stream 在不同层之间持续传递。由于每层新增更新的幅度不足以重写已经形成的 dominant spectral structure，token identity 对应的头部坐标得以跨层保留，并持续主导后续 gate。

我们对 Qwen3-0.6B 各层 raw residual input 的 centered activation matrix 分别提取第一主方向，并计算任意两层主方向的绝对余弦相似度。热力图显示，layer 3--27 之间形成了接近 $1$ 的高相似区域，说明这些层的 residual dominant direction 在深度方向上保持高度一致。

![Cross-layer similarity of raw residual dominant directions](../assets/residual_pc1_cross_layer_similarity_heatmap.png)

因此，浅层和深层 gate 会反复读取同一 token 持续存在的 dominant identity signal，形成较高的同-token跨层 expert activation 可预测性。这种可预测性来自 residual 强信号的跨层继承，并不等价于不同层已经形成了互补的 feature specialization。

#### 2.3.3 高连续性 Attention Output 为何未被当前 Gate 利用
...
前期 Oracle-MoE 工作已经指出，受 self-attention 加权聚合特性的影响，$A$ 在同一 sequence 的相邻 token 间具有较强连续性，能够产生稳定、可预测的 expert activation。进一步的谱空间分析显示，这种连续性主要集中在 $A$ 自身的主子空间：完整 $A$ 的 centered adjacent-token cosine 为 $0.518$，投影到自身 top $1\%$ 后进一步提升至 $0.645$。

然而，这部分高连续性信息加入 residual 后并未主导最终表征。Layer 1--27 的实测结果显示，$X$ 自身约 $90.78\%$ 的能量集中于 $H$ 的 top $1\%$；在最终 $H$ top $1\%$ 中，$X$ 的能量贡献达到 $98.68\%$，当前层 $A$ 仅占 $1.32\%$。因此，标准 MoE gate 读取完整 $H$ 时，主要响应累计 residual 的强方向，而无法直接利用 $A$ 主子空间中更连续的上下文信号。

![Attention continuity is masked by residual-dominated head directions](../assets/attention_residual_head_dominance.png)

由此，现有 gate 的预测结构可以由 residual 强方向的区分性与跨层继承解释；attention output 分析则揭示了进一步的结构机会：模型内部已经存在比完整 residual 更适合跨-token routing 的连续表征，可以作为后续 routing 结构的独立输入。
...
首轮小规模真实数据验证采用 4 层、$d=768$ 的模型。每层均包含 1 个常驻 common expert 和 4 个 long-tail experts，每个 token 激活 common expert 与其中 1 个 long-tail expert。结果如下：

| 模型 | 总参数 | 单 Token 激活参数 | Expert 数量 | 收敛 Loss | 每 100 Token 的 Expert 切换次数 |
|---|---:|---:|---:|---:|---:|
| Baseline MoE | 57.43M | 36.20M | 1 + 4 | 4.8342 | 62.13 |
| 本文结构 | 57.43M | 33.54M | 1 + 4 | 4.8768 | 6.30 (-89.90%) |

其中，Loss 为 step 5000 在固定测试样本上的 next-token loss；Expert 切换次数统计连续 100 个 token 在 4 层中发生的 routed expert 变化，不包含每层首次加载。本文结构的 routing 连续性采用已保留的 step 3000 快照。
