...
import train_scripts.train_crosstoken_Qwen30B as train_mod
...
def main():
    ...
    pred_model = CEPredictor(
        layers=train_mod.LAYER_NUM, input_dim=train_mod.HIDDEN_SIZE, num_experts=train_mod.NUM_EXPERTS
    ).to(train_mod.PREDICTOR_DEVICE)

    num_heads = train_mod._pick_heads(train_mod.HIDDEN_SIZE, int(getattr(cfg, "num_attention_heads", 16)))
    next_attn = train_mod.DeepNextTokenPredictor(
        train_mod.HIDDEN_SIZE, num_heads, num_layers=train_mod.NUM_LAYERS
    ).to(train_mod.PREDICTOR_DEVICE)

    ckpt_dir = f"../checkpoints/CEPredictor_Qwen3MoE_A3B_lr0.0001_Deep_{train_mod.NUM_LAYERS}L_top{train_mod.TOPK}"
    ...
    topk = int(os.environ.get("EVAL_TOPK", str(train_mod.TOPK)))
