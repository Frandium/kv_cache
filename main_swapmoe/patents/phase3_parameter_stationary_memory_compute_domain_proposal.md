...
标准 MoE 虽然每个 token 只激活少量 experts，但相邻 token 的 expert 身份频繁切换，瞬时稀疏访问不等于可由小容量高速存储长期承载。
**本项目的连续性增强路由算法改变了参数访问序列的时间局部性，使近期激活 experts 能够跨 token 持续复用。**
由此，端侧 MoE decode 形成如下容量与流量不对称：
...
3. 将本项目测得的 $92.70\%$ expert 驻留命中率作为条件输入，代入公开 30B-A3B MoE 配置构造的同规模参照模型后，估算 working set 可服务约 $95.49\%$--$96.95\%$ 的当前读取字节。
...
该架构只为高流量 working set 支付高带宽成本。为估计本项目模型未来扩展至 30B-A3B 规模时的下游部署效果，在 BF16、每层为 top-8 激活预留 16 个高速 expert slots 的同规模参照配置下，4K 和 32K context 对应的高速层容量比例约为 $16.4\%$ 和 $20.1\%$。本文进一步选取面向高端车载计算、机器人和边缘计算模组的代表性架构工作点：高速层带宽 1.5 TB/s、容量层带宽 273 GB/s、高速与容量型内存单位容量成本比 3。在“本项目模型扩展后仍保持当前驻留特性”的条件下，一阶带宽与端到端时延模型给出：
- 相比全 273 GB/s 容量型内存，存储子系统单位容量加权成本增加约 $40\%$，预计获得约 $2.95$--$3.04\times$ 的解码加速；
- 相比全 1.5 TB/s 高速存储，存储子系统成本降低约 $53.3\%$，预计解码吞吐损失约 $6.4\%$--$9.2\%$。
...
容量上的 20:80 分布不等于流量上的 20:80 分布。在后续同规模条件外推中，容量层虽然很大，但只需服务约 $3\%$--$4.5\%$ 的运行时字节；因此高速层可以显著降低相对全容量低速存储的供数时间。该流量比例是结构参照估算值，不是目标大模型的实测结果。

### 5.2 面向 30B-A3B 同规模扩展的高速层容量估算

本节用于回答：**如果本项目模型后续扩展到 30B 总参数、每 token 激活约 3B 参数的 MoE 规模，分层存储需要多大容量。** 为获得可复核的外部规模参照，本文选取 Qwen3-30B-A3B 的公开配置，其宏观结构参数为 48 层、hidden size 2048、128 个 routed experts、每 token 激活 8 个 experts、expert intermediate size 768、4 个 KV heads，BF16 参数量约为 30.53B。
...
该结构参照模型中，一个 expert 的 BF16 权重约为 9 MiB。若高速层为每层保留 16 个 expert slots，用于容纳 top-8 active experts 及切换余量，同时保存全部 attention/router 权重和输出 LM head，则对应高速权重约为：
...
该结构参照模型的 BF16 KV Cache 在全部 48 层的容量为：
...
因此，如果本项目模型未来达到上述结构规模，20% 高速容量与 80% 容量层可以作为 BF16、最高约 32K context 的首个容量设计参照。若未来同规模模型采用 INT4 权重而 KV 保持 BF16，32K context 下 KV 占比将上升，高速层比例约为 30.6%；专利可将高速层容量范围定义为总模型运行容量的 10%--40%，优选 20%--30%。

### 5.3 Expert 命中率与 Context Length 对高速存储覆盖率的影响

对于上述 30B-A3B 结构参照模型，每层 attention 权重约为 36 MiB，router 约为 0.5 MiB，8 个 active experts 共 72 MiB。每层历史 KV 读取量为：
...
![图2：30B-A3B 结构参照下 Expert 命中率与 Context Length 对高速存储覆盖率的影响](./figures/qwen3_fast_byte_coverage_vs_expert_hit.svg)
...
本项目自有模型的连续 routing 实验测得，相邻 token 的 expert stay probability 为 $92.70\%$。为了估计本项目模型未来扩展到上述 30B-A3B 规模后的硬件收益，本文进一步假设该驻留特性在扩展后能够保持，并将 $h=92.70\%$ 代入结构参照模型，得到 4K--32K context 下 $95.49\%$--$96.95\%$ 的高速存储覆盖率。该数值是“本项目实测驻留率 + 公开同规模结构参数”的条件外推结果，不是 Qwen3-30B-A3B 上的实测数据；后续应由本项目同规模目标模型的真实 expert slot 命中轨迹替换。

### 5.4 相对端侧低带宽基线的性能收益与成本增量

#### 5.4.1 公共假设

上述 30B-A3B 结构参照模型在 BF16 下加 KV 需要约 60 GiB。为估计本项目模型达到同等规模后的部署效果，并避免混用性能上限、手机典型值和高端统一内存值，本文只选取一个面向高端车载计算、机器人和边缘计算模组的代表性架构工作点：

$$
B_{\text{fast}}=1.5\ \text{TB/s},
\qquad
B_{\text{capacity}}=273\ \text{GB/s}.
$$

参数选择依据如下：

1. **高速层选择 1.5 TB/s。** FuriosaAI RNGD 推理 NPU 已采用两组 HBM3，在 48 GB 高速容量下实现 1.5 TB/s 聚合带宽；SK hynix 和 Micron 的单组 HBM3E 公开带宽也已达到约 1.18--1.2 TB/s 以上。这些产品证明 1.5 TB/s 对神经网络推理处理器不是不可实现的理论峰值。本文据此将 1.5 TB/s 作为高端边缘计算模组的高速层目标，而不是当前手机内存的典型值。RNGD 为 150 W 服务器推理卡，因此该引用只证明带宽工程可实现性，不证明约 12 GB、1.5 TB/s 的小容量配置已经存在；后者仍要求减少 stack height、定制 base die/接口或采用其他宽接口堆叠 DRAM。
2. **容量层选择 273 GB/s。** NVIDIA Jetson Thor 面向机器人和 physical AI，公开配置为 64/128 GB、256-bit LPDDR5X 和 273 GB/s 带宽，功耗范围 40--130 W；该配置同时具有端侧产品定位和承载约 60 GiB 模型的容量，因此比 546 GB/s 高端 PC 统一内存更适合作为车机/机器人容量层基线。在上述条件外推成立时，高速层估算覆盖约 $95.49\%$--$96.95\%$ 的读取字节，继续提高容量层带宽只能作用于剩余约 $3.05\%$--$4.51\%$ 流量。
3. **单位容量成本比选择 3。** NVIDIA 在 Grace CPU 内存方案比较中披露，四组 HBM2E 子系统的单位容量成本超过 DDR5 或 LPDDR5X 的 3 倍；本文取 $r=3$ 作为保守的系统级代表值。TrendForce 披露的 HBM3E 与服务器 DDR5 裸存储 ASP 差距曾为 4--5 倍，并预计随 2026 年市场供需变化收窄至 1--2 倍，说明采购价格具有周期性。因此，3 倍不是精确报价，而是用于架构估算的中心值，正文仍保留 $r=2$--$4$ 的敏感性范围。

该工作点不表示市场上已经存在“约 12 GB、1.5 TB/s”的标准端侧颗粒，而是假设通过定制高速堆叠或宽接口 DRAM，只为约 $20\%$ 的逻辑容量保留 1.5 TB/s 聚合带宽，其余约 $80\%$ 使用 273 GB/s 容量型内存。它优先对应高端车载计算、机器人和边缘计算模组；当前手机受持续功耗、封装尺寸和高速存储最小容量粒度限制，更适合采用较低带宽、较小容量的实施例。
...
成本采用一般化的一阶内存子系统模型。令容量型内存单位容量成本为 1，高速内存与容量型内存的单位容量成本比为 $r$：
...
r=3
\quad
\text{（敏感性范围 }r\in[2,4]\text{）}.
$$

这里的单位容量成本包括实现相应存储所必需的内存封装，不再在 $\delta$ 中重复计算同一高速存储的堆叠或基础封装成本。设高速层容量比例为 $f$，且两层容量互补、不重复保存高速层对象的完整后备副本，则分层方案相对全容量低速内存的成本比例为：
...
在中心工作点 $f=20\%$、$r=3$ 下，分层方案相对全容量低速内存的成本比例为 $1.4$，即增加约 $40\%$；相对全容量高速内存的成本比例为 $1.4/3=46.7\%$，即降低约 $53.3\%$。在 $r=2$--$4$ 的敏感性范围内，前者增加约 $20\%$--$60\%$，后者降低约 $40\%$--$60\%$。

上述比例只描述相关内存子系统，不等同于 NPU、计算模组、手机、车载计算单元或整机成本同比增加。实际增量还取决于两类控制器、PHY、额外集成封装、测试、良率和采购规模。

#### 5.4.2 结果

下表给出本项目模型未来扩展到 30B-A3B 同等规模时的条件性效果估算：采用本项目自有模型 Attention-Mean Routing 实验测得的 $h=92.70\%$ 作为驻留特性假设，采用公开同规模结构参数计算容量与流量，并使用 $\beta=0.5$ 将存储供数时延换算为端到端时延。容量层按可直接向计算阵列供数计算；成本采用 $f=20\%$、$r=3$。

| 工作点 | 相对全 273 GB/s 容量型内存 | 相对全 1.5 TB/s 高速存储 |
|---|---:|---:|
| 端到端解码性能 | 4K: **2.95× 加速**；32K: **3.04× 加速** | 4K: 时延 +10.1%、吞吐损失 9.2%；32K: 时延 +6.9%、吞吐损失 6.4% |
| 内存子系统成本 | **增加 40.0%** | **降低 53.3%** |

在本项目模型达到参照规模且保持当前驻留特性的条件下，只为约 $20\%$ 容量配置高速层，预计可相对全 273 GB/s 容量型内存端侧基线获得约 $2.95$--$3.04$ 倍的一阶端到端加速；相对全 1.5 TB/s 高速存储则预计降低约 $53.3\%$ 的内存子系统成本，吞吐损失约 $6.4\%$--$9.2\%$。该结果接近全高速上限的原因是条件外推中容量层只承担约 $3.05\%$--$4.51\%$ 的当前读取字节，而不是容量层本身接近高速层。具体结果仍需由本项目同规模目标模型、实际有效带宽、随机访问效率和系统中存储供数占比共同验证。

公开依据：

- [FuriosaAI RNGD：两组 HBM3、48GB、聚合 1.5 TB/s、150W](https://developer.furiosa.ai/latest/en/overview/rngd.html)
- [SK hynix HBM3E：单组 8-layer stack 达 1.18 TB/s](https://news.skhynix.com/en/rulebreakers-revolutions-design-scheme-elevates-hbm3e/)
- [Micron HBM3E：24/36GB，单组 stack 超过 1.2 TB/s](https://www.micron.com/products/memory/hbm/hbm3e)
- [NVIDIA Jetson Thor：64/128GB LPDDR5X、273 GB/s、40--130W](https://www.nvidia.com/en-us/autonomous-machines/embedded-systems/jetson-thor/)
- [NVIDIA Grace CPU 内存比较：HBM2E 单位容量成本超过 LPDDR5X 的 3 倍](https://developer.nvidia.com/blog/nvidia-grace-cpu-superchip-architecture-in-depth/)
- [TrendForce HBM3E/DDR5 价格倍数](https://www.trendforce.com/presscenter/news/20251218-12843.html)

#### 5.4.3 从内存子系统成本到计算模组与整机成本

设相关内存子系统占全容量低速端侧基线整机或计算模组成本的比例为 $\alpha_{\text{memory}}$，双控制器、额外 PHY 与额外集成成本占该基线总成本的比例为 $\delta$；$\delta$ 不再重复包含已经计入 $r$ 的高速存储基础封装成本。则：
...
因此，中心工作点下内存子系统成本增加 $40\%$ 不表示 NPU、计算模组或整机成本增加相同比例；该增量首先由 $\alpha_{\text{memory}}$ 稀释，再叠加双控制器、额外 PHY 与集成封装成本。本文不在缺少目标产品 BOM 的情况下预设 $\alpha_{\text{memory}}$ 和 $\delta$，而将其作为上会后由芯片与供应链团队标定的变量。
...
- 高速层聚合带宽目标为 1.5 TB/s；
- 容量层有效带宽目标为 273 GB/s；
- 高速与容量型内存单位容量成本比中心值为 3，敏感性范围为 2--4；
- 全容量高速性能上限中存储供数占端到端时延的比例取模拟值 $\beta=0.5$；
- 相对全容量低速内存增加的内存子系统与额外集成成本，能够由解码速度收益及整机价值覆盖。

该结果是一阶峰值带宽与内存子系统成本估算，不是整机实测结论。有效带宽、随机访问效率、迁移延迟、功耗、高速存储容量粒度、封装良率和真实 BOM 需要在芯片 cycle model、DRAMSim/Ramulator 模型及供应链成本数据中进一步标定。
...
3. 本项目自有模型已测得 $92.70\%$ 的 expert 驻留命中率；将该实测值作为条件输入，代入公开 30B-A3B 结构参照模型后，估算约 $16\%$--$20\%$ 的高速容量可覆盖约 $95.49\%$--$96.95\%$ 的当前读取字节；
...
6. 在 30B-A3B 规模下，如果本项目模型未来达到同等规模并保持当前 $h=92.70\%$ 的驻留特性，则在 1.5 TB/s 高速层、273 GB/s 容量层、$\beta=0.5$ 时，分层方案预计相对全容量 273 GB/s 内存的端侧基线获得约 $2.95$--$3.04$ 倍端到端加速，相对全容量 1.5 TB/s 高速存储的吞吐损失约为 $6.4\%$--$9.2\%$；
7. 当高速容量比例为 $20\%$、高速与容量型内存单位容量成本比取中心值 3 时，相关内存子系统成本相对全容量低速内存增加约 $40\%$，相对全容量高速内存降低约 $53.3\%$；该增量只作用于内存子系统，对计算模组和整机成本的影响还要乘以其 BOM 占比并计入双控制器与额外集成成本。
