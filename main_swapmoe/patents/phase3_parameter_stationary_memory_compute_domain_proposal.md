...
该架构只为高流量 working set 支付高带宽成本。以 30B-A3B 规模的 MoE 模型为例，在 BF16、每层为 top-8 激活预留 16 个高速 expert slots 的配置下，4K 和 32K context 对应的高速层容量比例约为 $16.4\%$ 和 $20.1\%$。在本文的带宽与端到端时延模型中，预计成本与时延收益为：
- 相比全低速存储，预计可以 $20\%$ 的存储介质成本增加，换取 $2.0\times$ 的解码加速；
- 相比全高速存储，预计可以降低 $60\%$ 存储介质成本，解码速度损失仅为 $9\%$。
